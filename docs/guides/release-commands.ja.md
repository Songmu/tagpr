...
標準出力と標準エラーは tagpr のログへ逐次出力されます。

いずれかのコマンドがゼロ以外のステータスで終了した場合、tagpr はその失敗を記録し、設定キー、コマンド、終了ステータス、標準エラーを含む GitHub Actions のエラー annotation を出力します。その後も残りのリリース準備処理は続行します。特に、`tagpr.command` が失敗しても、バージョンファイルの更新、`tagpr.postVersionCommand` の実行、リリースプルリクエストの作成または更新は継続されます。

リリースプルリクエストの処理が終了した後、tagpr は記録したコマンド失敗をエラーとして返し、ゼロ以外のステータスで終了します。両方のコマンドが失敗した場合や、後続の tagpr の処理も失敗した場合は、それらのエラーをまとめて報告します。そのため、リリースプルリクエストの作成または更新に成功していても GitHub Actions のステップは失敗扱いになります。マージする前に annotation とコマンド出力を確認してください。
